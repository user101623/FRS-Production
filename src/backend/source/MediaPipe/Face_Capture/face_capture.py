...
def crop_and_save(username, frame, coordinate, path, num_images, count=[0]):
    ...
    if count[0] == 0:
        image.save(os.path.join(path, f"{username}.jpg"))

    user_path = os.path.join(path, f"../Users/{username}")
    if not os.path.exists(user_path):
        os.makedirs(user_path)
    image.save(os.path.join(user_path, f"{username}_{count[0]:02d}.jpg"))
...
def validFace(frame, coordinate):
    ...
    if not (0 <= x < frame_width and 0 <= y < frame_height and x + w <= frame_width and y + h <= frame_height):
...
def save_face_landmarks_list_to_file(username, landmarksList, path):
    ...
    np.save(os.path.join(path, f"{username}.npy"), np_data)

def capture(name) -> Generator[bytes, None, None]:
    __location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
    dictionary = {"stopped": False}
    json_object = json.dumps(dictionary)

    json_dir = os.path.join(__location__, "../../../static/json")
    if not os.path.exists(json_dir):
        os.makedirs(json_dir)

    with open(os.path.join(json_dir, "streaming_data.json"), "w") as outfile:
